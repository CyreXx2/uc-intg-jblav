"""
JBL AV Receiver integration for Unfolded Circle Remote.
...
from intg_jblav.driver import JBLAVDriver
from intg_jblav.setup_flow import JBLAVSetupFlow
...
async def main():
    """Main entry point for JBL AV integration."""
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s | %(levelname)-8s | %(name)-20s | %(message)s"
    )

    _LOG.info("Starting JBL AV Receiver Integration v%s", __version__)
    ...
    try:
        ...
        config_manager = BaseConfigManager(
            ...
            config_class=JBLAVConfig,
        )
        driver.config_manager = config_manager

        # Create setup handler
        setup_handler = JBLAVSetupFlow.create_handler(driver)
        ...
        _LOG.info("JBL AV integration started successfully")
