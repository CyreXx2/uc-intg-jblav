...
from ucapi.ui import Buttons, DeviceButtonMapping, create_btn_mapping
...
# IMPORTANT:
# The Web Configurator expects UI pages to be JSON-serializable.
# Using ucapi.ui.Size(...) here breaks JSON serialization and crashes the driver.
...
class JBLAVRemote(Remote, Entity):
    ...
    def _create_ui_pages(self) -> list[dict[str, Any]]:
        ...
        NOTE: Must be JSON serializable (only dict/list/str/int/bool/None).
        """
        return [
    ...
    async def handle_command(
        ...
        try:
            ...
            # Audio settings
            if cmd_id == "TREBLE_UP":
                success = await self._device.set_treble_eq(3)
                return StatusCodes.OK if success else StatusCodes.SERVER_ERROR
            ...
            if cmd_id == "BASS_UP":
                success = await self._device.set_bass_eq(3)
                return StatusCodes.OK if success else StatusCodes.SERVER_ERROR
